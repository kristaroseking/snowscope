...
export async function POST(request: NextRequest) {
  try {
    const requests: SkiConditionRatingRequest[] = await request.json();
    ...
    const results = await Promise.all(
      requests.map(async (body) => {
        try {
          ...
          console.error("Error calculating score for hour:", error);
          // Return fallback score on error
          return {
            overall_score: 50,
            rating: "FAIR",
            rating_emoji: getRatingEmoji("FAIR"),
            component_scores: {
              new_snow: { score: 50, weight: 0, max_possible: 100, percentage: 50 },
              snow_depth: { score: 50, weight: 0, max_possible: 100, percentage: 50 },
              snow_quality: { score: 50, weight: 0, max_possible: 100, percentage: 50 },
              temperature: { score: 50, weight: 0, max_possible: 100, percentage: 50 },
              wind: { score: 50, weight: 0, max_possible: 100, percentage: 50 },
            },
            details: {
              conditions_summary: getConditionsSummary("FAIR"),
              best_for: [],
              considerations: [],
            },
          };
    ...
    console.error("Error in batch scoring API:", error);
    return NextResponse.json(
      {
        success: false,
        error:
          error instanceof Error
            ? error.message
            : "Failed to calculate ski condition scores",
      },
      { status: 500 }
    );
  }
